...
def correct_signature_data(signature_correction_file, output_file_name):
    #try:
    # 각인보정 파일 읽기
    correction_df = pd.read_excel(signature_data_file)
    correction_df = correction_df.applymap(str)
    data_df = pd.read_excel(signature_correction_file)
    ...
    # 수정된 내용을 새 파일로 저장
    data_df.to_excel(output_file_name, index=False)
